...
@Component({
  selector: 'app-wo-client-data',
  templateUrl: './wo-client-data.component.html',
  styleUrls: ['./wo-client-data.component.css'],
  providers: [
...
export class WoClientDataComponent implements OnInit {
  ...
  WOClientData() {
    ...
    let fileName = 'wo_client_data_'+ new DatePipe('en-US').transform(this.date.value.endOf('month'), 'MMM-yyyy').toUpperCase();
    this.reportsService.printAccountMonthlyReport(toDt, fileName , 9).subscribe((response) => {
      var binaryData = [];
