...
@Component({
  ...
  templateUrl: './rate-of-renewal.component.html',
  styleUrls: ['./rate-of-renewal.component.css'],
  providers: [
...
export class RateOfRenewalComponent implements OnInit {
  ...
  rateOfRenewalReport() {
    ...
    let dt = new Date(new DatePipe('en-US').transform(this.date.value.endOf('month'), 'yyyy-MM-dd'));
    ...
    this.operationsReportsService.printRateOfRenewalReport(new DatePipe('en-US').transform(dt, 'dd-MM-yyyy'), this.rpt_flg, areaSeq, regSeq, brnchSeq).subscribe((response) => {
      this.spinner.hide();
      var binaryData = [];
