...
@Component({
  selector: 'app-wo-recovery',
  templateUrl: './wo-recovery.component.html',
  styleUrls: ['./wo-recovery.component.css'],
  providers: [
...
export class WoRecoveryComponent implements OnInit {
  ...
  WORecovery() {
    ...
    let fileName = 'wo_recovery_'+ new DatePipe('en-US').transform(this.date.value.endOf('month'), 'MMM-yyyy').toUpperCase();
    this.reportsService.printAccountMonthlyReport(toDt, fileName, 7).subscribe((response) => {
      var binaryData = [];
