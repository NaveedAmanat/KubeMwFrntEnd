...
@Component({
  selector: 'app-acc-topsheet',
  templateUrl: './acc-topsheet.component.html',
  styleUrls: ['./acc-topsheet.component.css'],
  providers: [
...
export class AccTopsheetComponent implements OnInit {
  ...
  topsheet() {
    ...
    let fileName = 'topsheet_' + new DatePipe('en-US').transform(this.date.value.endOf('month'), 'MMM-yyyy').toUpperCase();
    this.reportsService.printAccountMonthlyReport(toDt, fileName , 4).subscribe((response) => {
      var binaryData = [];
