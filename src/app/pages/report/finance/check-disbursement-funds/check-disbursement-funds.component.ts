...
@Component({
  selector: 'app-check-disbursement-funds',
  templateUrl: './check-disbursement-funds.component.html',
  styleUrls: ['./check-disbursement-funds.component.css'],
  providers: [
  { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
  { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
],
})
export class CheckDisbursementFundsComponent implements OnInit {
  ngForm: FormGroup;
  frmDt: string;
  toDt: string;
  maxDate: Date;
  ...
  mcbRemitanceDisb() {
    ...
    console.log(isXls);
    this.financeService.printChequeDisbFunds(frmDt, toDt, isXls).subscribe((response) => {
      this.spinner.hide();
      var binaryData = [];
