...
@Component({
  selector: 'app-consolidated-bank-funds',
  templateUrl: './consolidated-bank-funds.component.html',
  styleUrls: ['./consolidated-bank-funds.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class ConsolidatedBankFundsComponent implements OnInit {
  ...
  printReport() {
    ...
    this.financeService.printConsolidatedOfBankFunds(frmDt, toDt, isXls).subscribe((response) => {
      this.spinner.hide();
      var binaryData = [];
  ...
  postFunds(){
    ...
    this.financeService.consolidatedOfBankFundsLoader(frmDt, toDt).subscribe((response) => {
      this.spinner.hide();
      this.displayUploader = false;
      if(response['success']){
        this.toaster.success(response['success'], "Success");
      }else if(response['failed']){
        this.toaster.info(response['failed'], "Info");
      }
    }, (error) => {
      this.spinner.hide();
      if (error.status == 500) {
