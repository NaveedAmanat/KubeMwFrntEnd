...
@Component({
  selector: 'app-bmmobile-wallet-trend',
  templateUrl: './bmmobile-wallet-trend.component.html',
  styleUrls: ['./bmmobile-wallet-trend.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
  })
export class BMmobileWalletTrendComponent implements OnInit {
  ngForm: FormGroup;
  frmDt: string;
  toDt: string;
  maxDate: Date;
  ...
  dateWiseDisb() {
    ...
    let type: boolean = this.ngForm.get('type').value;
    this.financeService.printMobileWalletTrend(frmDt, toDt, brnchSeq, type, isXls).subscribe((response) => {
      this.spinner.hide();
      var binaryData = [];
