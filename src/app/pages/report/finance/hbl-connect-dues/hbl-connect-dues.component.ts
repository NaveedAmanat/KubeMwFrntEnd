...
@Component({
  selector: 'app-hbl-connect-dues',
  templateUrl: './hbl-connect-dues.component.html',
  styleUrls: ['./hbl-connect-dues.component.css'],
  providers: [
    { provide: DateAdapter, useClass: MomentDateAdapter, deps: [MAT_DATE_LOCALE] },
    { provide: MAT_DATE_FORMATS, useValue: MY_FORMATS },
  ],
})
export class HblConnectDuesComponent implements OnInit {
  ngForm: FormGroup;
  // frmDt: string;
  toDt: string;
  maxDate: Date;
  ...
  hblConnectDues() {
    ...
    console.log(isXls);
    this.financeService.printHblConnectDues( toDt, true).subscribe((response) => {
      this.spinner.hide();
      var binaryData = [];
