...
@Component({
  selector: 'app-premium-data',
  templateUrl: './premium-data.component.html',
  styleUrls: ['./premium-data.component.css'],
  providers: [
...
export class PremiumDataComponent implements OnInit {
  ...
  premiumData() {
    ...
    let fileName = 'premium_data_'+ new DatePipe('en-US').transform(this.date.value.endOf('month'), 'MMM-yyyy').toUpperCase();;
    this.reportsService.printAccountMonthlyReport(toDt, fileName , 3).subscribe((response) => {
      var binaryData = [];
