...
@Component({
  selector: 'app-kszb-client-data',
  templateUrl: './kszb-client-data.component.html',
  styleUrls: ['./kszb-client-data.component.css'],
  providers: [
...
export class KszbClientDataComponent implements OnInit {
 ...
 ngForm: FormGroup;
  ...
  KSZBClientData() {
    ...
    let fileName = 'kszb_client_data_'+ new DatePipe('en-US').transform(this.date.value.endOf('month'), 'MMM-yyyy').toUpperCase();
    this.reportsService.printAccountMonthlyReport(toDt, fileName, 5).subscribe((response) => {
      var binaryData = [];
