...
@Component({
  ...
  styleUrls: ['./acc-recovery.component.css'],
  providers: [
...
export class AccRecoveryComponent implements OnInit {
  ...
  accRecovery() {
    ...
    let fileName = 'recovery_data_'+ new DatePipe('en-US').transform(this.date.value.endOf('month'), 'MMM-yyyy').toUpperCase();
    this.reportsService.printAccountMonthlyReport(toDt, fileName , 8).subscribe((response) => {
      var binaryData = [];
