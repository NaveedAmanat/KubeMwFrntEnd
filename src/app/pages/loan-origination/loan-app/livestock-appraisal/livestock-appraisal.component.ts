...
import { EstimatedLivestock } from 'src/app/shared/models/EstimatedLivestock.model';
import { ExistingLivestock } from 'src/app/shared/models/ExistingLivestock.model';
...
@Component({
  ...
  templateUrl: './livestock-appraisal.component.html',
  styleUrls: ['./livestock-appraisal.component.css']
})
export class LivestockAppraisalComponent implements OnInit {
  ...
  calculateTotalValues() {
    ...
    this.totalExtHc = 0;
    this.totalExtVal = 0;
    for (let i = 0; i < this.model.businessAppraisal.extngLvStk.length; i++) {
      const x = +this.model.businessAppraisal.extngLvStk[i].anmlHc;
      const y = +this.model.businessAppraisal.extngLvStk[i].estVal;
      this.totalExtHc = this.totalExtHc + x;
      this.totalExtVal = this.totalExtVal + y;
    }

    this.totalEstHc = 0;
    this.totalEstVal = 0;
    this.totalClntFunded = 0;
    this.totalKashfFunded = 0;
    for (let i = 0; i < this.model.businessAppraisal.estLvStk.length; i++) {
      const x = +this.model.businessAppraisal.estLvStk[i].anmlHc;
      const y = +this.model.businessAppraisal.estLvStk[i].estVal;
      const a = +this.model.businessAppraisal.estLvStk[i].amtByClient;
      const b = +this.model.businessAppraisal.estLvStk[i].amtFin;
      this.totalEstHc = this.totalEstHc + x;
      this.totalEstVal = this.totalEstVal + y;
      this.totalClntFunded = this.totalClntFunded + a;
      this.totalKashfFunded = this.totalKashfFunded + b;
    }

    this.estimatedValPercent();
    this.incomeChange();
  }
  ...
  estimatedValPercent(){
    this.clntFunded = ((this.totalClntFunded * 100) / this.totalEstVal).toFixed(2);
    this.kashfFunded = ((this.totalKashfFunded * 100) / this.totalEstVal).toFixed(2);
  }

  ngOnInit() {
    this.livestockForm = this.fb.group({
      anmlKnd: ['', Validators.required],
      anmlBrd: ['', Validators.required],
      anmlHc: ['', Validators.required],
      estVal: ['', Validators.required],
      anmlTyp: ['', Validators.required]
    });
    this.financeForm = this.fb.group({
      ...
      estVal: ['', Validators.required],
      anmlTyp: ['', Validators.required],
      amtByClient: ['', Validators.required],
      amtFin: ['', Validators.required]
    });
    ...
    this.hasPermission = this.commonService.checkPermission('livestock-appraisals', this.model);
   // end
  }
  ...
  onBusinessAppraisalFormSubmit() {
    ...
    if (this.model.businessAppraisal.extngLvStk == null || this.model.businessAppraisal.extngLvStk == undefined || this.model.businessAppraisal.extngLvStk.length <= 0) {
        ...
        $('#livestockid').click();
        this.toaster.error("Existing livestock is Missing."); this.spinner.hide();
        return;
    ...
    if (this.model.businessAppraisal.estLvStk == null || this.model.businessAppraisal.estLvStk == undefined || this.model.businessAppraisal.estLvStk.length <= 0) {
      ...
      $('#financeid').click();
      this.toaster.error("Estimated livestock is Missing."); this.spinner.hide();
      return;
  ...
  estLvStk: EstimatedLivestock = new EstimatedLivestock();
  extngLvStk:  ExistingLivestock = new ExistingLivestock();
  ...
  mfcibArray: any;
  arrayOfLiveStockTyp: any;
  arrayOfLiveStockknd: any;
  ...
  arrayOfLiveStockkndOrg: any;
  arrayOfLiveStockTypOrg:any;
  ...
  loadLovs() {
    ...
    // Modified by Areeba - 9-12-2022 - KMWM Product
    if (this.model.loanProdGrp == 24) {
      this.commonService.getValues(REF_CD_GRP_KEYS.ANIMAL_STABLE_LOCATION).subscribe((res) => {
    ...
    this.commonService.getValues(REF_CD_GRP_KEYS.BUSINESS_RUNNER).subscribe((res) => {
      ...
      console.log("BUSINESS_RUNNER:", res);
      ...
      this.loanService.getClntRel(this.model.nominee).subscribe((clntRelRes) => {
        console.log("Client Relation:", clntRelRes);
    ...
    this.commonService.getValues(REF_CD_GRP_KEYS.HOUSEHOLD_EXPENSES_FOR_LIVESTOCK).subscribe((res) => {
      ...
      this.loanService.getMfcibLoans(this.model.loanAppSeq).subscribe((res) => {
        this.mfcibArray = res;
        let key = this.findSeqFromCd("0010", this.houseHoldExpenseTypeOrig);
        ...
        console.log('otherLoanAmountPerMonth', this.otherLoanAmountPerMonth)
        if (this.otherLoanAmountPerMonth > 0) {
          ...
          if (key >= 0) {
            ...
            console.log('houseHoldExpenseTypeOrig' , this.houseHoldExpenseType);
            console.log('houseHoldExpenseTypeOrig', this.houseHoldExpenseTypeOrig);
          }
    ...
    let expense_group = REF_CD_GRP_KEYS.BUSINESS_EXPENSES;
    ...
    this.loanService.getMwPrdBySeq(this.model.loanProd).subscribe(res => {
      if (res != null)
        ...
        if (res.prdGrpSeq == 5305)
          expense_group = REF_CD_GRP_KEYS.LIVESTOCK_EXPENSES_MEAT;
          
        // Modified by Areeba - 9-12-2022 - KMWM Product
        else if (res.prdGrpSeq == 16 || res.prdGrpSeq == 24)
          expense_group = REF_CD_GRP_KEYS.LIVESTOCK_EXPENSES_DAIRY;

      this.model.loanProd == this.commonService.getValues(expense_group).subscribe((res) => {
    ...
    this.model.loanProd == this.commonService.getValues(REF_CD_GRP_KEYS.LIVESTOCK_ANML_TYP).subscribe((res) => {
      this.arrayOfLiveStockTypOrg = JSON.parse(JSON.stringify(res));
        this.arrayOfLiveStockTyp = res;
    ...
    this.model.loanProd == this.commonService.getValues(REF_CD_GRP_KEYS.LIVESTOCK_ANML_KND).subscribe((res) => {
      this.arrayOfLiveStockkndOrg = JSON.parse(JSON.stringify(res));
      this.arrayOfLiveStockknd = res;
  ...
  removeItemsFromLOVUsingBizApp() {
    ...
    this.model.businessAppraisal.extngLvStk.forEach(res => {
      this.removeItemFromLOV(res.anmlTyp, this.arrayOfLiveStockTyp);
    });
    this.model.businessAppraisal.estLvStk.forEach(res => {
      this.removeItemFromLOV(res.anmlTyp, this.arrayOfLiveStockTyp);
    });
  ...
  incomeChange() {
    ...
    if (Number.isNaN(this.averageIncome))
  ...
  openLivestock() {
    this.isEdit = false;
    this.extngLvStk = new ExistingLivestock();
  ...
  onSubmitLivestock() {
    (<any>$('#livestock')).modal('hide');
    const result: ExistingLivestock = Object.assign({}, this.livestockForm.value);
    if (!this.isEdit) {
  ...
  onEditLivestock(extngLvStk,index) {
    this.removeItemsFromLOVUsingBizApp();
    this.arrayOfLiveStockTypOrg.forEach(element => {
      if (element.codeKey == extngLvStk.anmlTyp) {
        this.arrayOfLiveStockTyp.push(element);
      }
    });
  ...
  onDeleteLivestock(stock,index) {
    swal({
      ...
      if (result.value) {
        this.removeItemsFromLOVUsingBizApp();
        this.model.businessAppraisal.extngLvStk.splice(index, 1);
        swal(
          'Deleted!',
  ...
  openFinance() {
    this.isEdit = false;
    this.estLvStk = new EstimatedLivestock;
    this.financeForm.reset();
    this.removeItemsFromLOVUsingBizApp();
    (<any>$('#finance')).modal('show');
  }

  onEditFinance(estLvStk,index) {
    (<any>$('#finance')).modal('show');
  ...
  onDeleteFinance(fin) {
    swal({
      ...
      if (result.value) {
        this.model.businessAppraisal.estLvStk.splice(this.model.businessAppraisal.estLvStk.indexOf(fin), 1);
        swal(
          'Deleted!',
          'Deleted Successfully.',
          'success'
  ...
  onSubmitFinance() {
    (<any>$('#finance')).modal('hide');
   const result: EstimatedLivestock = Object.assign({}, this.financeForm.value);
   if (!this.isEdit) {
